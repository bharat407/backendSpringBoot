  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredShows = showRes.data.filter(
          (s) => s.event?.id === parseInt(eventId),
        );
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
        ...
        <div className="flex flex-wrap gap-4 text-gray-600 text-sm">
          <span className="flex items-center">
            <MapPin className="w-4 h-4 mr-1" /> {event.city}
          </span>
          <span className="flex items-center">
            <Clock className="w-4 h-4 mr-1" /> {event.durationMinutes} mins
          </span>
      ...
      {shows.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
          <p className="text-gray-500">
            No shows available for this event yet.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="space-y-3 mb-6">
                ...
                <div className="flex items-center text-sm text-gray-600 bg-gray-50 p-2 rounded-lg">
                  ...
                  <span>
                    {show.totalSeats - (show.bookedSeats || 0)} seats available
                  </span>
                </div>
              </div>
              ...
              <Button
                onClick={() =>
                  navigate(`/book/${show.id}`, { state: { show, event } })
                }
                disabled={
                  isAdmin || show.totalSeats - (show.bookedSeats || 0) <= 0
                }
                className="w-full"
              >
                {isAdmin
                  ? "Admin View Only"
                  : show.totalSeats - (show.bookedSeats || 0) <= 0
                    ? "Sold Out"
                    : "Book Seats"}
              </Button>
            </motion.div>
